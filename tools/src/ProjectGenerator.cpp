...
// DESIGN NOTE: CMakeLists.txt is always regenerated by trusscli.
// User-specific CMake config should go in local.cmake (auto-loaded by trussc_app()).
// CMakePresets.json holds paths and build presets (also regenerated).
...
string ProjectGenerator::update(const string& projectPath_) {
    ...
    try {
        ...
        log("Writing CMakeLists.txt...");
        {
            ...
            cmake << "# TRUSSC_DIR is provided by CMakePresets.json (generated by trusscli)\n";
            cmake << "# For CI/manual builds without presets, fallback to relative path\n";
            cmake << "if(NOT DEFINED TRUSSC_DIR)\n";
            cmake << "    set(TRUSSC_DIR \"${CMAKE_CURRENT_SOURCE_DIR}/../../../core\")\n";
            cmake << "endif()\n\n";
