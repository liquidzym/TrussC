...
- `example-core-pingpong`, `example-optical-flow`, `example-fluid-bridges`, `example-camera-fluid`, `example-particles`, `example-wind-tunnel`, and `example-hd`.
- Basic tests for settings, resize, density injection, and procedural optical-flow state.

Still limited:

- LIC/streamlines, full split-velocity shader parity, advanced bridge controls such as invert/alpha/mirror options, and PixelFlow-style particle variants remain pending.
...
- Visualization: `shaders/visualization/visualization.glsl` with scalar, velocity color, pressure, temperature, and combined fluid passes.
