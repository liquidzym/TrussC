import { createGesture, serializeGestureResult } from "../tasks/gesture";
import { RateCounter } from "../stats";
import type { Delegate, WorkerInboundMessage } from "../types";

let task: Awaited<ReturnType<typeof createGesture>>["task"] | null = null;
...
self.onmessage = async (event: MessageEvent<WorkerInboundMessage>) => {
  ...
  if (message.type === "config") {
    try {
      const created = await createGesture(message.delegate, message.maxGestures);
      task = created.task;
      ...
      self.postMessage({
        ...
        wasmPath: "/wasm",
        models: { hand: false, pose: false, face: false, gesture: true }
      });
    } catch (error) {
      self.postMessage({
        type: "runtime_status",
        ...
        reason: error instanceof Error ? error.message : "GestureRecognizer setup failed",
        wasmPath: "/wasm",
